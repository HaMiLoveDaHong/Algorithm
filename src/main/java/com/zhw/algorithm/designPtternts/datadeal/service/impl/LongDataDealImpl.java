 ...
 * @sice 2022/7/5
 **/
public class LongDataDealImpl implements DataDealService {
    @Override
    public DataAfterVo interceptor(DataDealChain chain) {
        DataBeforeVo dataBeforeVo = chain.beforeVo();
        if (DataBeforeVo.StatusConst.THREE.equals(dataBeforeVo.getStatus())){
            System.out.println("执行 LongDataDealImpl 业务");
            return new DataAfterVo("执行 LongDataDealImpl 业务");
        }
